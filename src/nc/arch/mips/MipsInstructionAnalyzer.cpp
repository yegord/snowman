...
NC_DEFINE_REGISTER_EXPRESSION(MipsRegisters, cflag) // virtual 1-bit register for conditional branching
...
private:
    void createStatements(MipsExpressionFactoryCallback & _, const MipsInstruction *instruction, core::ir::Program *program) {
        ...
        _[regizter(MipsRegisters::zero()) ^= constant(0)];
        ...
        case MIPS_INS_ABS: {
            MipsExpressionFactoryCallback _then(factory, program->createBasicBlock(), instruction);
            MipsExpressionFactoryCallback _else(factory, program->createBasicBlock(), instruction);
            _[
                jump((signed_(operand(1)) < signed_(constant(0))),
                     (_then[operand(0) ^= -operand(1), jump(directSuccessor())]).basicBlock(),
                     (_else[operand(0) ^= operand(1), jump(directSuccessor())]).basicBlock())
            ];
            break;
        }
    ...
    std::unique_ptr<core::ir::Term> createTermForOperand(const cs_mips_op &operand, SmallBitSize sizeHint) const {
        switch (operand.type) {
            case MIPS_OP_REG: {
                ...
                //}
                return std::make_unique<core::ir::MemoryLocationAccess>(reg.resized(sizeHint));
            }
            case MIPS_OP_IMM: {
                /* Immediate value. */
