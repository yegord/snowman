...
struct elf_aarch64_link_hash_table
{
  ...
  /* A table of fix locations for erratum 835769.  This holds erratum
     fix locations between elf32_aarch64_size_stubs() and
...
static struct elf_aarch64_stub_hash_entry *
elf32_aarch64_add_stub (const char *stub_name,
			asection *section,
			struct elf_aarch64_link_hash_table *htab)
{
  asection *link_sec;
	   ...
	   input_bfd != NULL; input_bfd = input_bfd->link.next, bfd_indx++)
	{
	  ...
	  for (section = input_bfd->sections;
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  stub_entry = elf32_aarch64_add_stub (stub_name, section,
						       htab);
		  if (stub_entry == NULL)
		    {
		      free (stub_name);
	...
	if (h && h->type == STT_FUNC)
	  {
		...
		struct elf_aarch64_stub_hash_entry *stub_entry;
		stub_entry = elf32_aarch64_get_stub_entry (input_section,
							   sym_sec, h,
							   rel, globals);
		if (stub_entry != NULL)
...
static bfd_boolean
elf32_aarch64_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword out_flags;
  flagword in_flags;
...
static bfd_boolean
elf32_aarch64_gc_sweep_hook (bfd *abfd,
			     struct bfd_link_info *info,
			     asection *sec,
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      else
	{
	  ...
	  if (isym != NULL
	    ...
	    {
	      h = elf32_aarch64_get_local_sym_hash (htab, abfd, rel, FALSE);
	      if (h == NULL)
		abort ();
	    }
	}
...
static bfd_boolean
elf32_aarch64_output_plt_map (struct elf_link_hash_entry *h, void *inf)
{
						   ...
						   *))
{
  ...
  elf_link_hash_traverse (&htab->root, elf32_aarch64_output_plt_map,
			  (void *) &osi);
